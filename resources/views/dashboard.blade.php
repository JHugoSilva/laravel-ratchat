    ...
    <div class="row">
        ...
        <div class="col-sm-4 col-lg-6">
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        <div class="col col-md-6" id="chat_header"><b>Chat Área</b></div>
                        <div class="col col-md-6" id="close_chat_area"></div>
                    </div>
                </div>
                <div class="car-body" id="chat_area"></div>
            </div>
        </div>
        <div class="col-sm-4 col-lg-3">
...
<script>
    ...
    conn.onmessage = function(e) {
                ...
                html += '<ul class="list-group">'
                    for (var count = 0; count < data.data.length;count++) {
                        ...
                        if (data.data[count].user_image != '') {
                            ...
                            user_image = `<img src="{{ asset('images/default.jpg') }}" width="40" class="rounded-circle"/>`
                        }
                        ...
                        html += `
                            <li class="list-group-item">
                                <div class="row">
                                    ...
                                    <div class="col col-3">
                                        <button type="button" name="send_request" class="btn btn-primary btn-sm float-end" onclick="send_request(this,`+from_user_id+`, `+data.data[count].id+`)">
        ...
        if (data.reponse_from_user_chat_request) {
        ...
        if (data.response_connected_chat_user) {
            ...
            var html = '<div class="list-group">'
                if (data.data.length > 0) {
                    for (var count = 0; count < data.data.length; count ++) {
                        html += `<a href="#" class="list-group-item d-flex justify-content-between align-items-start" onclick="make_chat_area('`+data.data[count].id+`','`+data.data[count].name+`'); load_chat_data(`+from_user_id+`, `+data.data[count].id+`)">
                            ...
                            `
                                ...
                                if (data.data[count].user_image != '') {
                                    ...
                                    user_image = `<img src="{{ asset('images/default.jpg') }}" width="35" class="rounded-circle"/>`
                                }
        ...
        if (data.message) {
            ...
            if (data.from_user_id == from_user_id) {
                ...
                if (data.message_status == 'Not Send') {
                    icon_style = `
                        <span id="chat_status_`+data.message_status+`" class="float-end">
                        ...
                        <div class="row">
                            ...
                            if (data.data[count].notification_type == 'Send Request') {
                                if (data.data[count].status == 'Pending') {
                                    ...
                                    html += '<button type="button" name="send_request" class="btn btn-danger btn-sm float-end">Request Reject</button>'
                                }
                            } else {
                                if (data.data[count].status == 'Pending') {
                                    html += '<button type="button" class="btn btn-danger btn-sm float-end"><i class="fas fa-times" onclick="proccess_chat_request('+data.data[count].id+', '+data.data[count].from_user_id+', '+data.data[count].to_user_id+',`Reject`)"></i></button>&nbsp'
                                    html += '<button type="button" class="btn btn-success btn-sm float-end"><i class="fas fa-check" onclick="proccess_chat_request('+data.data[count].id+', '+data.data[count].from_user_id+', '+data.data[count].to_user_id+',`Approve`)"></i></button>'
                                } else {
                                    html += '<button type="button" name="send_request" class="btn btn-danger btn-sm float-end">Request Reject</button>'
                                }
                            }
        ...
        if (data.chat_history) {
            ...
            for (var count = 0; count < data.chat_history.length; count++) {
                ...
                if (data.chat_history[count].message_status == 'Not Send') {
                    icon_style = `
                        <span id="chat_status_`+data.chat_history[count].id+`" class="float-end">
                            <i class="fas fa-check text-muted"></i>
                        </span>
                    `
                }
                ...
                if (data.chat_history[count].message_status == 'Read') {
                    icon_style = `
                        <span id="chat_status_`+data.chat_history[count].id+`">
                            <i class="fas fa-check-double text-muted"></i>
                        </span>
                    `
                }
                    ...
                    html += `
                        <div class="row">
                            <div class="col col-9 alert alert-success text-dark shadow-sm float-end">
                    ...
                    if (data.chat_history[count].message_status != 'Read') {
                        ...
                        html += `
                            <div class="row">
                                <div class="col col-9 alert alert-light text-dark shadow-sm float-end">
        ...
        if (data.update_message_status) {
            ...
            if (data.unread_msg) {
                ...
                if (count_unread_message_element) {
                    ...
                    count_unread_message_element.innerHTML = '<span class="badge text-bg-danger">'+count_unread_message+'</span>'
                }
    ...
    function proccess_chat_request(chat_request_id, from_user_id, to_user_id, action) {
        var data = {
    ...
    function make_chat_area(user_id, to_user_name) {
        var html =`
            ...
            <div class="input-group mb-3">
                ...
                <button type="button" class="btn btn-success" id="send_button" onclick="send_chat_message()">
                    <i class="fas fa-paper-plane"></i>
                </button>
            </div>
        `
        ...
        document.getElementById('close_chat_area').innerHTML = `
            <button class="btn btn-danger btn-sm float-end" onclick="close_chat()">
